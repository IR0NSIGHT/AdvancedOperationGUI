...
const ConfigBuilder: React.FC = () => {
  ...
  const handleLayerChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setGlobalWPOperation({
      ...globalWPOperation,
      layer: [selectedLayer, selectedValue],
    });
  };

  const layerOptions = Array.from({ length: 16 }, (_, index) => index).map(
    (value) => (
    ...
    <div>
      ...
      <div>
        ...
        <select
          ...
          value={globalWPOperation.layer[0]}
        >
          {layerOptions}
        </select>
      </div>
      <div>{/* Skip onlyOnLayer and perlin for now */}</div>
      <div>
