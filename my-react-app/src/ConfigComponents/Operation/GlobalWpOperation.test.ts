import {applyLayerChange, DisplayOperation, emptyOperation, GlobalWpOperation} from './GlobalWpOperation';
...
describe('changeWriteLayer function', () => {
    ...
    it('add new layer', () => {
        const layers: WpLayerSetting[] = [['Frost', 1], ['Pines', 3]];
    ...
    it('mutate existing layer value', () => {
        const layers: WpLayerSetting[] = [['Frost', 1], ['Pines', 3]];
        expect(layers).toEqual([['Frost', 1], ['Pines', 3]])
    ...
    it('does not allow double entries', () => {
        const layers: WpLayerSetting[] = [['Pines', 7], ["Annotations", 12]];
        const updatedOp = applyLayerChange(layers!, ['Pines', 7], ['Annotations', 7]);

        expect(updatedOp).toEqual([['Annotations', 7]])
    });

    it('overwrites old and new layer matches', () => {
        const layers: WpLayerSetting[] = [['Pines', 7], ['Pines', 2], ["Annotations", 12], ["Annotations", 2], ["Burgers", 3]];
        const updatedOp = applyLayerChange(layers, ['Pines', 7], ['Annotations', 7]);
